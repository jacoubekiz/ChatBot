...
def handel_request_redis(self, data: str):
    ...
    try:
        ...
        if statuses:
            ...
            channel = Channle.objects.filter(phone_number=display_phone_number).select_related('account_id').first()
            if not channel:
                return {'error': 'Channel not found'}
            
            chatmessage = ChatMessage.objects.select_related('conversation_id').get(wamid=message_id)
            ...
            if contacts:
                ...
                # Use caching for restart keywords to avoid repeated database queries
                cache_key = f'restart_keywords_{channel.channle_id}'
                restart_keywords = cache.get(cache_key)
                if restart_keywords is None:
                    ...
                    cache.set(cache_key, restart_keywords, timeout=3600)  # Cache for 1 hour
                ...
                if conversation.state == 'start_bot':
                    ...
                    match content_type:
                        ...
                        case "image":
                            ...
                            headers = {'Authorization': f'Bearer {channel.tocken}'}
                            response = _http_session.get(f"https://graph.facebook.com/v15.0/{image_id}", headers=headers, timeout=30)
                        ...
                        case "video":
                            ...
                            # Use connection pooling and timeout
                            headers = {'Authorization': f'Bearer {channel.tocken}'}
                            response = _http_session.get(f"https://graph.facebook.com/v15.0/{video_id}", headers=headers, timeout=30)
                        ...
                        case "audio":
                            ...
                            # Use connection pooling and timeout
                            headers = {'Authorization': f'Bearer {channel.tocken}'}
                            response = _http_session.get(f"https://graph.facebook.com/v15.0/{audio_id}", headers=headers, timeout=30)
                        ...
                        case 'document':
                            ...
                            # Use connection pooling and timeout
                            headers = {'Authorization': f'Bearer {channel.tocken}'}
                            response = _http_session.get(f"https://graph.facebook.com/v15.0/{document_id}", headers=headers, timeout=30)
